...
7. **Assemble a code-first app to generate model predictions** from sample scenic images and text descriptions. Streamlit app code examples can be edited easily for different use cases! If you have created your own deployment in Datarobot (based on your own trails data or above data sample), simply clone this repository to get access to "requirements.txt" file, the "Hiking_app.py" file, and "project_metadata.py" file setup. More details on steps to follow for this setup our in "Hiking_app.py" file.
8. Let's Make Predictions now to see what the model recommends - [Hiking Application](https://share.streamlit.io/1arjunarora/hikingapp/main/Hiking_app.py)
